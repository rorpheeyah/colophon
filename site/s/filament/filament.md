...
This project follows Filament. The rules in @.claude/design-system.md are
...
A filament is a metal thread that glows when current runs through it and is dark when it does
not. That is the system, and it is also why both of its edges are named the way they are.
...
is no, the motion is decoration and this system does not permit it. See section 8.
...
Several ideas here were taken from published developer-console work: a border that travels
around the element it belongs to, chrome that contracts into a floating capsule as you scroll,
frosted translucency for anything content passes beneath, and a near-black pill as the primary
action. Those are behaviours, and behaviours are portable.

What follows is what this system does differently, stated so a reader can see the work was done
rather than assumed:

| Axis | There | Here |
|---|---|---|
| **Primitive** | A travelling border is decoration on a call to action | A travelling border is a state, and appears **only** when the element is live |
| **Meaning** | One animation, one appearance | Two moving edges with two meanings — steel invites, purple reports |
| **Palette** | A purple brand bolted onto stock framework neutrals, so warm ground carries cool grey type | Every neutral is a step of the same ramp and carries its hue |
| **Naming** | Tokens named `gunmetal` and `iron-grey` both point at purples | A token is named for its role or its hue, and never for a colour it is not |
| **Modes** | A dark palette declared in scattered per-utility literals | Dark is a token layer, and the mode is the only thing that switches |
...
The most important row is the first. A decorative animation cannot be extrapolated — an agent
reading *"the button has a moving border"* learns nothing about the next screen. A semantic one
can: *"is this live? then its edge moves"* answers every case that will ever come up. Promoting
the move from decoration to rule is the whole reason this system exists rather than being a
restyle.
...
:root {
  ...
  --clp-alarm:          var(--ember);
  --clp-alarm-wash:     none;
  --clp-invert-bg:      var(--prunella);
  --clp-invert-text:    var(--on-prunella);
...
the block above redeclares values, and nothing anywhere else in the system carries a
mode-specific literal. A dark mode assembled out of per-element overrides is not a mode, it is a
patch, and it will drift.
...
**`prunella` is the same value in both modes.** The brand ground does not change when the lights
go out. It is the one token that is mode-independent, and that is deliberate.
...
**Every neutral is a step of the purple ramp.** `wash`, `panel`, `ink`, `ink-2`, `ink-3` and
`rule` all carry the same hue as `prunella`. There are no stock grey neutrals here, because a
warm ground carrying cool grey type never reconciles — the two read as two systems sharing a
screen.

**The metal ramp is the one exception, and it carries no hue at all.** Every step of `steel-*`
has equal red, green and blue channels. That is what keeps it a *material* rather than a second
brand colour, and it is a rule an agent can check by reading the hex. See section 9.

### Colour rules

| Token | Means |
|---|---|
| `prunella` | The ground — nav rail, inverted panels, the brand surface in both modes |
...
Every token above is named for its role or its hue. **None is named for a colour it is not.**
That sounds obvious and is the failure this system most deliberately avoids: a token called
`gunmetal` that holds a purple will be reached for by the wrong person on the wrong element, and
no amount of documentation undoes a name that lies.

**One purple ramp, one metal, one exception.** Every hue in this system is a step of the purple
ramp except `ember`. A reader who sees any non-purple *hue* already knows what it means before
reading the label: something has failed.
...
**Khmer is not a translation layer bolted on at the end.** Every text style in the table above
has a Khmer line-height, and a screen is not finished until it has been read in both. Developer
tooling built where Khmer is spoken and shipped Latin-only is a reach failure, not a scope
decision.
...
is what proves it is above the page rather than part of it. See section 8 for the timing.
...
| Nav rail | `prunella` ground, `--clp-invert-text`, active item marked by `--clp-invert-accent` |
...
- **Never name a token for a colour it is not.** A token holding a purple is never called
  `gunmetal`, `slate`, `iron` or any other name that describes a different colour. The name is
  what someone reaches for; a name that lies puts the wrong value on the element.
- **Never introduce a stock grey neutral.** Every neutral is a step of the purple ramp and
  carries its hue. A warm ground under cool grey type never reconciles.
- **Never assemble dark mode out of per-element overrides.** Dark is the token block and nothing
  else. A mode-specific literal anywhere outside it is a patch, and it will drift.
- **Never declare a dark token the mode does not actually use.** A dark palette that never
  renders is worse than none, because it reads as finished.
- **Never ship Latin-only.** Khmer is declared in `scripts`, every text style carries a Khmer
  line-height, and a screen is not done until it has been read in both.
...
- **Never put glass on anything that content does not pass beneath.** A card, a table, a form
  panel, a sidebar and a modal are all `panel`, however good a frosted one would look.
